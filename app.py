...
AOI: draw a rectangle on the map OR enter lat/lon.
...
# ── Session state ─────────────────────────────────────────────────────────────
# All image data stored as bytes (PNG) so it survives st.rerun() serialization.
for k, v in {
    "naip_img_bytes": None,   # PNG bytes of the fetched tile
    "naip_b64": None,         # base64 string for VLM API
    "naip_scene": None,
    "messages": [],
    # Indexes
    ...
    # Results — all stored as serialization-safe types
    "result_map_html": None,
    "result_chip_bytes": [],   # list of PNG bytes
    "result_scores": [],
    "result_indices": [],
    ...
    # Pending search (avoids st.rerun loop)
    "pending_search": None,    # {"query": str, "intent": str} | None
}.items():
...
<b>Draw:</b> Rectangle tool on the map tab.<br>
<b>Point:</b> Lat/Lon tab.<br><br>
<b>Chat:</b> "What land cover is present?"<br>
<b>Visual search:</b> "Find all parking lots"<br>
<b>Spectral:</b> "Where is NDVI highest?"
...
def chip_to_bytes(chip_float32):
    """Convert (3,H,W) float32 [0,1] chip to PNG bytes."""
    rgb = (chip_float32.transpose(1,2,0) * 255).clip(0,255).astype(np.uint8)
    return pil_to_bytes(Image.fromarray(rgb))
...
def fetch_naip_point(lat, lon, buf):
    return _fetch_common([lon-buf, lat-buf, lon+buf, lat+buf])

def fetch_naip_bbox(bbox):
    return _fetch_common(bbox)
...
def ensure_indexes(item):
    ...
    if vc and sc:
        ...
        if not vc:
            ...
            save_index(fidx, ip)
            save_meta({"positions":positions,"chip_gdf":chip_gdf}, mp)
        else:
        ...
        if not sc:
            ...
            save_spectral_embeddings(sembs, sep)
            save_spectral_index(sidx, sip)
        else:
            chips4=load_chips(cp4); sembs=load_spectral_embeddings(sep)
            sidx=load_spectral_index(sip)

    # Store numpy arrays in session state — fine here since they're not
    # crossing a rerun boundary (search runs synchronously in same render pass)
...
# ── Store results (all serialization-safe) ────────────────────────────────────
def _store_results(idxs, scores, chip_gdf, chips, spec_reports=None):
    # Convert chips to PNG bytes immediately — avoids numpy serialization issues
    chip_bytes = [chip_to_bytes(chips[i]) for i in idxs]
    st.session_state.result_indices          = list(idxs)
    st.session_state.result_scores           = list(scores)
    st.session_state.result_chip_bytes       = chip_bytes
            ...
            r=client.chat.completions.create(
                ...
                messages=[{"role":"user","content":[
                    {"type":"text","text":(
                        ...
                        f'Each tile labelled with chip index e.g. #42. '
                        f'Which single chip index best matches? Reply ONLY the number.')},
    ...
    if client:
        try:
            ...
            for k,idx in enumerate(idxs[:strip_n]):
                strip.paste(Image.fromarray(
                    (chips[idx].transpose(1,2,0)*255).astype(np.uint8)),(k*chip_size,0))
...
# ══════════════════════════════════════════════════════════════════════════════
# LAYOUT — three sections: AOI, results, chat (no st.rerun needed)
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("---")

# ── Section 1: AOI selection (two tabs) ──────────────────────────────────────
aoi_col, info_col = st.columns([3, 1], gap="large")
...
with aoi_col:
    ...
    with latlon_tab:
        st.caption("Enter a point coordinate and buffer.")
...
with info_col:
    st.markdown("#### Status")
    if st.session_state.naip_img_bytes:
        item = st.session_state.naip_scene
        st.success("Tile loaded")
        if item:
            st.caption(f"Scene: {item.id[:30]}...")
            st.caption(f"Date: {item.datetime.date()}")
        if st.session_state.faiss_idx is not None:
            n = len(st.session_state.chips)
            st.info(f"{n} chips indexed")
    else:
        st.info("No tile loaded yet.")
...
with chat_col:
    ...
    prompt = st.chat_input(
        ...
        disabled=(not st.session_state.naip_img_bytes))
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            if intent == "SPECTRAL":
                if not item:
                    response = "Load a NAIP tile first."
                    st.markdown(response)
                else:
                    with st.spinner("Running spectral search..."):
                        response = run_spectral_search(prompt, item)
                    st.markdown(response)
                    st.markdown("_Matching chip images and index cards are shown on the left ←_")
                ...
                if not item:
                    ...
                    st.markdown(response)
                    st.markdown("_Matching chip images are shown on the left ←_")

            else:
                response = run_chat(prompt)

        # Append both messages and let Streamlit re-render naturally
        # NO st.rerun() — the results are already in session state and
        # the img_col block above will pick them up on the next natural render
